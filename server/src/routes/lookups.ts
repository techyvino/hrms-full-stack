...
lookupRouter.post("/add-role", async (c) => {
  ...
  const isRoleExist = await db.select().from(roleTypeTable).where(eq(roleTypeTable.name, body.name))
...
lookupRouter.post("/add-site", async (c) => {
  ...
  const isExist = await db.select().from(sitesTable).where(eq(sitesTable.name, body.name))
...
lookupRouter.get("/", async (req) => {
  return req.json({ message: "Hello lookup!" })
})
